...
class SwerveModuleIOFalconPro(
  ...
  private val potentiometerOutput: Double
    get() {
      return if (label != Constants.Drivetrain.BACK_RIGHT_MODULE_NAME) {
        ...
        2 * Math.PI - potentiometer.voltage / RobotController.getVoltage5V() * 2.0 * Math.PI
      }
    }

  private val openLoopOutput = DutyCycleOut(0.0)

  init {
    openLoopOutput.EnableFOC = true
  ...
  override fun setOpenLoop(steering: Angle, speed: LinearVelocity) {
    openLoopOutput.Output = speed / DrivetrainConstants.DRIVE_SETPOINT_MAX
    driveFalcon.setControl(openLoopOutput)
    setSteeringSetpoint(steering)
  }
