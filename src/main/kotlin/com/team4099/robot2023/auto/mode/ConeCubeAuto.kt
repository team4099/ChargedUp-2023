...
class ConeCubeAuto(val drivetrain: Drivetrain, val superstructure: Superstructure) :
  ...
  companion object {
    ...
    val startingPosY =
      LoggedTunableValue(
        "Drivetrain/startingPosY1", 4.97.meters, Pair({ it.inMeters }, { it.meters })
      )
    val startingPosTheta =
      LoggedTunableValue(
        "Drivetrain/startingPosTheta1", 180.0.degrees, Pair({ it.inDegrees }, { it.degrees })
      )
    ...
    val endingPosY =
      LoggedTunableValue(
        "Drivetrain/endingPosY1", 4.97.meters, Pair({ it.inMeters }, { it.meters })
      )
    val endingPosTheta =
      LoggedTunableValue(
